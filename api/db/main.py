...
conninfo = f'host={os.getenv("DB_HOST")} port={os.getenv("DB_PORT")} dbname={os.getenv("DB_NAME")} user={os.getenv("DB_USER")} password={os.getenv("DB_PASS")}'


async def delete_all_tables():
    aconn = await psycopg.AsyncConnection.connect(conninfo)
    delete_all_query = """
...
async def create_user(
    user_name: str, registration_record: str, rsa_key: str, salt: str, iv: str
):
    """Insert user record"""
    aconn = await psycopg.AsyncConnection.connect(conninfo)
    insert_query = """
    ...
    async with aconn:
        async with aconn.cursor() as curr:
            insert = await curr.execute(
                insert_query, (user_name, registration_record, rsa_key, salt, iv)
            )
...
async def update_chat(user_id: int, chat_id: int, messages_json: str):
    try:
        ...
        async with aconn:
            async with aconn.cursor() as curr:
                chats = await curr.execute(
                    update_query, (messages_json, chat_id, user_id)
                )
                result = await chats.fetchone()
                if not result:
                    logging.error(
                        f"This chat ID {chat_id} does not belong to user ID {user_id}"
                    )
    except ValidationError as e:
        logging.error(e.errors())
...
async def create_login_record(user_name: str, login_record: str, login_time: str):
    ...
    async with aconn:
        async with aconn.cursor() as curr:
            records = await curr.execute(
                insert_query, (user_name, login_record, login_time)
            )
...
async def create_credential_secret(user_name: str, cred_sec: str, login_time: str):
    ...
    async with aconn:
        async with aconn.cursor() as curr:
            secrets = await curr.execute(
                insert_query, (user_name, cred_sec, login_time)
            )
